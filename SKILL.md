...
description: Install, bootstrap, adopt, repair, upgrade, validate, or remove the ACHP harness-agnostic agent collaboration scaffold in a repository. Use only when the user explicitly asks to set up or change the collaboration mechanism itself. Do not use for normal project planning, coding, reviews, handoffs, Git synchronization, or knowledge maintenance after setup.
license: MIT
compatibility: Agent Skills compatible harness with repository read/write access; Python 3.9+ is recommended for deterministic setup scripts.
metadata:
  version: "0.1.0"
  scope: "setup-only"
  protocol: "ACHP"
...
This skill is an **installer/configurator**, not the collaboration runtime.
...
- `uninstall`: remove ACHP-managed setup while preserving project knowledge by default.
...
After installation, fill `.agents/coordination/PROJECT.md` with only facts supported by the repository or explicitly supplied by the user. Mark unknowns as unknown instead of inventing them.
