...
listed by `ScenarioCatalog.TESTS` in `tools/runtime/p1_profile_probe.py`.
It does not write a formal Gate. The current profile inventory is 12 runnable
scenarios and 6 `NOT_RUN` gaps; `P1-IDENTITY-CONTINUITY` is the twelfth
runnable adapter. The command is generated by
`p1_profile_plan.py --profile ... --plan ... --status ...`, so
`ScenarioCatalog`/`LINEAGE_BOUND`/`MODEL_REQUIREMENTS` and the availability
calculation are authoritative rather than this inventory text. A runnable
scenario can still be `NOT_RUN` in a particular execution when its required
same-run host resources are absent.
...
and Identity continuity use the actual `acs-delivery/<operation>` Workflow and
persist its run ID in the Domain operation; these are the two real-delivery
workflow exceptions in the current catalog.
...
`p1_profile_plan.py` emits six commands only for the twelve runnable scenarios
when their required real resources are current, and an empty command list for
every catalog gap. Missing command kinds therefore stay `NOT_RUN`; the harness
never fabricates a passed result. The command and six readbacks must share one
